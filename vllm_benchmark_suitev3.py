...
class SystemInfo:
    ...
    def get_gpu_name() -> Optional[str]:
        """Get GPU model name(s) with full detail."""
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                if len(names) == 1:
                    return names[0]
                # Group identical GPUs with a count prefix
                from collections import Counter
                gpu_counts = Counter(names)
                parts = []
                for gpu_name, count in gpu_counts.items():
                    if count > 1:
                        parts.append(f"{count}x {gpu_name}")
                    else:
    ...
    def get_total_vram() -> Optional[float]:
        """Get total GPU VRAM in GB across all devices."""
        try:
            result = subprocess.run(
                [
                    "nvidia-smi",
            ...
            if result.returncode == 0:
                vrams = [float(v) for v in result.stdout.strip().split("\n") if v.strip()]
                if not vrams: return None
                return sum(vrams) / 1024  # Convert MB to GB
        except Exception:
            pass
        return None
...
class GPUMonitor:
    ...
    def get_gpu_stats(self) -> Optional[Dict]:
        """
        Query nvidia-smi for current GPU statistics across all devices.
        Averages utilization and temperature, sums power and memory.

        Returns:
            Dictionary containing aggregated GPU metrics or None if query fails
        """
        try:
            result = subprocess.run(
                [
            ...
            if result.returncode == 0:
                ...
                # Aggregate metrics
                return {
                    "gpu_util": mean([m["gpu_util"] for m in all_metrics]),
                    "mem_used": sum([m["mem_used"] for m in all_metrics]),
                    "mem_total": sum([m["mem_total"] for m in all_metrics]),
                    "temperature": mean([m["temperature"] for m in all_metrics]),
                    "power_draw": sum([m["power_draw"] for m in all_metrics]),
                    "gpu_clock": mean([m["gpu_clock"] for m in all_metrics]),
                    "mem_clock": mean([m["mem_clock"] for m in all_metrics]),
                    "timestamp": time.time(),
                    "num_gpus": len(all_metrics)
                }
        except Exception as e:
            # Only print error periodically to avoid spamming the console
            if not hasattr(self, "_last_error_time") or time.time() - self._last_error_time > 10:
...
def safe_load_tokenizer(model_name: Optional[str] = None) -> Tuple[Optional[Any], bool]:
    """
    Attempt to load a tokenizer with multiple fallbacks.
    Returns (tokenizer, is_heuristic)
    """
    # 1. Try specified model
    if model_name:
        try:
            tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True, timeout=5)
            return tokenizer, False
        except Exception:
            pass

    # 2. Try default tokenizer (gpt2)
    try:
        tokenizer = AutoTokenizer.from_pretrained(DEFAULT_TOKENIZER, timeout=5)
        return tokenizer, False
    except Exception:
        pass

    # 3. Fallback to None (caller should use heuristic)
    return None, True
...
def generate_deterministic_prompt(target_tokens: int, tokenizer_model: str) -> str:
    ...
    tokenizer, is_heuristic = safe_load_tokenizer(tokenizer_model)
    ...
    def get_token_count(text):
        if is_heuristic or tokenizer is None: return len(text) // 4
        return len(tokenizer.encode(text))

    base_tokens = get_token_count(query_text) + get_token_count(story_text)
    if base_tokens > target_tokens: return query_text # minimize if too long

    #if enough room to repeat, calculate repetitions
    n_remaining_tokens = target_tokens - base_tokens
    story_tokens = get_token_count(story_text)
    repetitions = max(0, n_remaining_tokens // story_tokens) if story_tokens > 0 else 0
...
def generate_madlib_prompt(target_tokens: int, tokenizer_model: str) -> str:
    ...
    def get_token_count(text):
        if is_heuristic: return len(text) // 4
        return len(tokenizer.encode(text))
    ...
    base_tokens = get_token_count(query_text) + get_token_count(story_text)
    if base_tokens > target_tokens: return query_text
    
    #if enough room to repeat, calculate repetitions
    buffer_tokens = int(target_tokens * 0.01)
    n_remaining_tokens = target_tokens - (base_tokens + buffer_tokens)
    
    one_story_tokens = get_token_count(story_text)
    repetitions = max(0, n_remaining_tokens // one_story_tokens) if one_story_tokens > 0 else 0
...
def generate_random_prompt(target_tokens: int, tokenizer_model: str) -> str:
    ...
    query_tokens = get_token_count(query_text)
    n_remaining_tokens = target_tokens - (query_tokens + buffer_tokens)
    random_text = make_random_text(max(0, n_remaining_tokens))
...
def visualize_results(
    ...
    # Layout: Dynamic based on features
    # Row 0: Throughput landscape (Always)
    # Row 1: Heatmaps (Always)
    # Row 2: Performance metrics line plots (Always)
    # Row 3: Efficiency & Decode plots (Always)
    # Row 4: GPU metrics (Conditional - Skip if no GPU stats)
    # Row 5: Prompt type comparison (Conditional - Skip if < 2 prompt types)
    # Row 6+: Cache heatmaps (Conditional)

    has_gpu_stats = "avg_gpu_util" in df_main.columns
    has_prompt_types_comp = "prompt_type" in df.columns and df["prompt_type"].nunique() >= 2
    has_cache_stats = "cache_hit_rate" in df.columns
    need_cache_rows = has_cache_stats
    
    rows = []
    rows.append({"type": "landscape", "name": "throughput"}) # Row 0
    rows.append({"type": "heatmaps", "name": "main_heatmaps"}) # Row 1
    rows.append({"type": "grid", "name": "perf_metrics"})     # Row 2
    rows.append({"type": "grid", "name": "efficiency"})       # Row 3
    
    if has_gpu_stats:
        rows.append({"type": "grid", "name": "gpu_metrics"})
    
    if has_prompt_types_comp:
        rows.append({"type": "landscape", "name": "prompt_comparison"})
        
    cache_row_start = len(rows)
    if need_cache_rows:
        num_pts = len(df["prompt_type"].unique())
        cache_count = (num_pts + 1) // 2
        for i in range(cache_count):
            rows.append({"type": "cache_heatmaps", "name": f"cache_{i}", "index": i})

    num_rows = len(rows)
    height_ratios = [1.2 if r["type"] == "landscape" else 1.0 for r in rows]
    # Adjust ratios for cache heatmaps (usually slightly shorter)
    for i, r in enumerate(rows):
        if r["type"] == "cache_heatmaps":
            height_ratios[i] = 0.8

    fig_height = sum([5 * r for r in height_ratios]) + 2
    
    fig = plt.figure(figsize=(24, fig_height))
    # Increase top margin to avoid title overlap
    gs = fig.add_gridspec(
        num_rows, 3, hspace=0.45, wspace=0.32, left=0.06, right=0.98, top=0.92, bottom=0.04,
    ...
    # ROW 0: Throughput vs Context Length (Landscape)
    ...
    ax1.set_title("Throughput vs Context Length by Concurrency", fontsize=15, fontweight="bold", pad=15)
    ...
    ax2 = fig.add_subplot(gs[current_row, :2])
    ...
    ax2.set_title("Throughput Heatmap (Tokens per Second)", fontsize=13, fontweight="bold", pad=10)
    ...
    ax3.set_title("Latency Heatmap (Total Seconds)", fontsize=13, fontweight="bold", pad=10)
    ax3.set_yticklabels([f"{int(y / 1000)}K" for y in pivot_latency.index], rotation=0, fontsize=9)
    current_row += 1

    # ROW 2: Primary Performance Metrics
    # GRAPH 4: Throughput per User
    ax4 = fig.add_subplot(gs[current_row, 0])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
        ax4.plot(
    ...
    ax5 = fig.add_subplot(gs[current_row, 1])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
        ax5.plot(
    ...
    # GRAPH 6: Average Latency
    ax6 = fig.add_subplot(gs[current_row, 2])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
        ax6.plot(
    ...
    # ROW 3: Generation Efficiency
    # GRAPH 7: Prompt Processing Speed
    ax7 = fig.add_subplot(gs[current_row, 0])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
        ax7.plot(
    ...
    ax7.set_ylabel("Prefill Speed (tokens/sec)", fontsize=11, fontweight="bold")
    ...
    # GRAPH 8: Inter-Token Latency
    ax8 = fig.add_subplot(gs[current_row, 1])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
        ax8.plot(
    ...
    ax8.axhspan(0, 50, alpha=0.1, color='green')
    ax8.axhspan(50, 100, alpha=0.1, color='yellow')
    ax8.axhspan(100, 200, alpha=0.1, color='orange')
    ax8.set_xlabel("Context Length (K tokens)", fontsize=11, fontweight="bold")
    ax8.set_ylabel("ITL (ms)", fontsize=11, fontweight="bold")
    ax8.set_title("Inter-Token Latency (UX Quality)", fontsize=12, fontweight="bold", pad=10)
    ax8.set_xticks([c / 1000 for c in context_lengths])
    ax8.set_xticklabels(context_labels)
    ax8.grid(True, alpha=0.3, linestyle="--")
    ax8.set_facecolor("#FAFAFA")

    # GRAPH 9: Decode Speed
    ax9 = fig.add_subplot(gs[current_row, 2])
    for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
        data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
    ...
    ax9.set_ylabel("Tokens/sec", fontsize=11, fontweight="bold")
    ax9.set_title("Decode Speed (Generation)", fontsize=12, fontweight="bold", pad=10)
    ...
    # ROW 4: GPU METRICS (Optional)
    if has_gpu_stats:
        # GRAPH 10: Power Draw
        ax10 = fig.add_subplot(gs[current_row, 0])
        for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
            data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
            ax10.plot(data["context_length"] / 1000, data["avg_power"],
                    marker="o", linewidth=2.5, markersize=9, label=f"{users} users",
                    color=colors[idx % len(colors)], markeredgecolor="white", markeredgewidth=1.5)
        ax10.set_xlabel("Context Length (K tokens)", fontsize=11, fontweight="bold")
        ax10.set_ylabel("Power (W)", fontsize=11, fontweight="bold", color="#E63946")
        ax10.set_title("Average Power Draw", fontsize=12, fontweight="bold", pad=10)
        ax10.set_xticks([c / 1000 for c in context_lengths])
        ax10.set_xticklabels(context_labels)
        ax10.tick_params(axis="y", labelcolor="#E63946")
        ax10.grid(True, alpha=0.3, linestyle="--")

        # GRAPH 11: GPU Clock
        ax11 = fig.add_subplot(gs[current_row, 1])
        for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
            data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
            ax11.plot(data["context_length"] / 1000, data["avg_gpu_clock"],
        ...
        ax11.set_ylabel("Clock (MHz)", fontsize=11, fontweight="bold")
        ax11.set_title("GPU Clock Frequency", fontsize=12, fontweight="bold", pad=10)
        ax11.set_xticks([c / 1000 for c in context_lengths])
        ax11.set_xticklabels(context_labels)
        ax11.grid(True, alpha=0.3, linestyle="--")

        # GRAPH 12: Scaling Efficiency
        ax12 = fig.add_subplot(gs[current_row, 2])
        for idx, users in enumerate(sorted(df_main["concurrent_users"].unique())):
            if users == 1: continue
            data = df_main[df_main["concurrent_users"] == users].sort_values("context_length")
            ax12.plot(data["context_length"] / 1000, data["batch_efficiency"],
        ...
        ax12.set_ylabel("Efficiency (%)", fontsize=11, fontweight="bold")
        ax12.set_title("Batch Scaling Efficiency", fontsize=12, fontweight="bold", pad=10)
        ax12.set_xticks([c / 1000 for c in context_lengths])
        ax12.set_xticklabels(context_labels)
        ax12.grid(True, alpha=0.3, linestyle="--")
        current_row += 1

    # ROW 5: Prompt Type Comparison (Optional)
    if has_prompt_types_comp:
        ax13 = fig.add_subplot(gs[current_row, :])
        ...
        for idx, ptype in enumerate(sorted(comparison_data["prompt_type"].unique())):
            ...
            y_data = data["actual_prefill_time"] if "actual_prefill_time" in data.columns and data["actual_prefill_time"].sum() > 0 else data["prefill_time_estimate"]
            ax13.plot(data["context_length"] / 1000, y_data, marker="o", linewidth=3, markersize=10, 
                     label=ptype.capitalize(), color=prompt_colors.get(ptype, colors[idx % len(colors)]), 
                     markeredgecolor="white", markeredgewidth=1.5)
        
        ax13.set_xlabel("Context Length (K tokens)", fontsize=13, fontweight="bold")
        ax13.set_ylabel("Prefill Time (s)", fontsize=13, fontweight="bold")
        ...
        ax13.legend(title="Prompt Type", loc="best", frameon=True, shadow=True)
        ax13.grid(True, alpha=0.3, linestyle="--")
        current_row += 1

    # ROW 6+: Cache Hit Rate Heatmaps (Optional)
    if need_cache_rows:
        prompt_types_list = sorted(df["prompt_type"].unique())
        for idx, ptype in enumerate(prompt_types_list):
            cache_row = current_row + (idx // 2)
            cache_col = (idx % 2) * 2
            col_span = 2 if (idx % 2 == 0 and idx == len(prompt_types_list) - 1) else (2 if idx % 2 == 0 else 1)
            if idx == len(prompt_types_list) - 1 and idx % 2 == 0: col_span = 3

            if col_span == 2: ax_cache = fig.add_subplot(gs[cache_row, cache_col:cache_col+2])
            elif col_span == 3: ax_cache = fig.add_subplot(gs[cache_row, :])
            else: ax_cache = fig.add_subplot(gs[cache_row, 2])
            
            ptype_data = df[df["prompt_type"] == ptype]
            pivot_cache = ptype_data.pivot(index="context_length", columns="concurrent_users", values="cache_hit_rate")
            sns.heatmap(pivot_cache, annot=True, fmt=".1f", cmap="RdYlGn", ax=ax_cache,
                        cbar_kws={"label": "Hit Rate (%)", "shrink": 0.8}, vmin=0, vmax=100)
            ax_cache.set_title(f"Cache Hit Rate: {ptype.capitalize()}", fontsize=12, fontweight="bold", pad=10)
            ax_cache.set_yticklabels([f"{int(y / 1000)}K" for y in pivot_cache.index], rotation=0, fontsize=8)

    # HEADER & SUBTITLE Formatting
    # Improve horizontal space usage and include model parameters
    header_info = []
    if system_info:
        if system_info.get("gpu_name"):
            vram = system_info.get("total_vram_gb")
            gpu_desc = f"{system_info['gpu_name']}" + (f" ({vram:.0f}GB)" if vram else "")
            header_info.append(f"GPU: {gpu_desc}")
    
    if server_info:
        if server_info.get("version"): header_info.append(f"vLLM: {server_info['version']}")
        if server_info.get("quantization"): header_info.append(f"Quant: {server_info['quantization']}")
        
        # Model Parameters (requested)
        params = []
        if server_info.get("tensor_parallel"): params.append(f"TP={server_info['tensor_parallel']}")
        if server_info.get("pipeline_parallel"): params.append(f"PP={server_info['pipeline_parallel']}")
        if server_info.get("max_num_seqs"): params.append(f"MaxBat={server_info['max_num_seqs']}")
        if server_info.get("gpu_memory_utilization"): params.append(f"MemUtil={server_info['gpu_memory_utilization']:.0%}")
        if params: header_info.append(f"Params: {', '.join(params)}")

    # Split subtitle into two lines if too long, or use a wide spread
    line1 = " | ".join(header_info[:3])
    line2 = " | ".join(header_info[3:])
    subtitle = line1 + ("\n" + line2 if line2 else "")
    subtitle += f"\nOutput: {output_tokens} tokens | Timestamp: {datetime.now().strftime('%Y-%m-%d %H:%M')}"

    fig.suptitle(f"{model_name} - Performance Report", fontsize=18, fontweight="bold", y=0.985)
    fig.text(0.5, 0.945, subtitle, ha="center", fontsize=12, fontweight="medium", 
...
def get_interactive_config() -> Tuple[List[int], List[int], int, List[str]]:
    ...
    all_standard_contexts = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
    ...
    console.print("  [2] 4 users")
    console.print("  [3] 16 users")
    console.print("  [4] 64 users (default)")
    console.print("  [5] 128 users")
    console.print("  [6] 256 users")
    ...
    if user_choice == 7:
        # Custom users
        max_users = IntPrompt.ask("Enter max concurrent users", default=64)
    else:
        user_map = {1: 1, 2: 4, 3: 16, 4: 64, 5: 128, 6: 256}
        max_users = user_map[user_choice]
    
    # Generate user counts up to max (powers of 2)
    all_user_levels = [1, 2, 4, 8, 16, 32, 64, 128, 256]
...
def create_live_dashboard(
    ...
    """
    # Dynamically adjust remaining tests panel size
    max_queue_items = 10
    remaining_size = min(max(8, min(len(remaining_tests), max_queue_items) + 4 if remaining_tests else 6), 18)
    ...
    if remaining_tests and len(remaining_tests) > 0:
        queue_text = Text()
        queue_text.append(f"Queue ({len(remaining_tests)} more tests):\n", style="bold blue")
        
        # Only show the next few items to keep display clean
        display_count = min(len(remaining_tests), max_queue_items)
        for i in range(display_count):
            ctx, users, ptype = remaining_tests[i]
            queue_text.append(f"  → {ctx // 1000}K × {users} users ({ptype})\n", style="dim")
        
        if len(remaining_tests) > max_queue_items:
            queue_text.append(f"  ... and {len(remaining_tests) - max_queue_items} more tests", style="italic gray")
        
        layout["remaining"].update(
            Panel(queue_text, title=f"Upcoming Workload", border_style="blue")
        )
    else:
        layout["remaining"].update(
